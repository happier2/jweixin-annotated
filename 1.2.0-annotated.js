(function jWeixin(window, initFunc) {
  ...
  function addVerfiyInfo(config) { // every invoke must have these verify info
    config = config || {}
    config.appId = rawConfig.appId
    config.verifyAppId = rawConfig.appId
    config.verifySignType = 'sha1'
    config.verifyTimestamp = rawConfig.timestamp + ""
    config.verifyNonceStr = rawConfig.nonceStr
    config.verifySignature = rawConfig.signature
  ...
  const wx = {
    ...
    error: function(callback) {
      if (clientVersion <= '6.0.2') {
        bridgeState.state == -1 ? callback(bridgeState.data) : callbackManager._fail = a
      }
    },
    checkJsApi: function(config) {
      let normalizeResult = function(result) { // replace methodName to commandName
        var resultObj = result.checkResult
        for (var key in resultObj) {
          var commandName = commandNameMap[key]
          if (commandName) {
    ...
    getNetworkType: function(config) {
      let normalizeResult = function(result) {
        ...
        if(type) {
          ...
          const index = errMsg.indexOf(':')
          const RESULT = errMsg.substring(index + 1)

          switch(RESULT) {
            case 'wifi':
            case 'edge':
            case 'wwan':
              result.networkType = result
              break
            default:
